...
class FunctionCall:
    ...
    def from_local_cbor(self, cborData, localCborLoader):
        ...
        callee_local_id = cborData['callee_local_id']
...
class Function:
    ...
    def from_local_cbor(self, cborData, localCborLoader, assign_id):
        ...
        if assign_id:
            self.assign_global_id(self.hash(), localCborLoader, cborData['local_id'])

        localCborLoader.current_tu.add_function(self, cborData['local_id'])
...
class CXXClass:
    ...
    def from_local_cbor(self, cborData, localCborLoader):
        ...
        localCborLoader.current_tu.add_class(self, cborData['local_id'])
    ...
    def load_local_cbor(self, cborData):
        ...
        if 'stuff' in cborData:
            ...
            for stuff in cborData['stuff']:
                if 'type' in stuff:
                    if stuff['type'] == StuffType_RecordDecl:
                        ...
                        if self.add_class(cxxclass):
                            for method in cxxclass.methods:
                                self.add_function(method)
                    elif stuff['type'] == StuffType_FunctionDecl:
                        func = Function(self.globalAST)
                        func.from_local_cbor(stuff, self, True)
                        self.add_function(func)
                    elif stuff['type'] == StuffType_MethodDecl:
                        # These here are problably template instantiations
                        cxx_class = self.current_tu.cxx_class(stuff['class_local_id'])
                        method = CXXMethod(self.globalAST, cxx_class)
                        method.from_local_cbor(stuff, self)
                        if self.add_function(method):
                            cxx_class.methods.append(method)
            # Process CallExprs
            for stuff in cborData['stuff']:
                if 'type' in stuff and stuff['type'] == StuffType_CallExpr: # CallExpr
...
def calculate_call_counts(ast):
    ...
    for c in ast.function_calls:
        if c.callee_global_id in ast.function_map:
            ast.function_map[c.callee_global_id].num_called += 1
        else:
...
#print_calls(globalAST, 'toChanged')

calculate_call_counts(globalAST)
print_signal_counts(globalAST)

#print("Printing QObjects")
#print_qobjects(_globalAST)
#print ("num functions=" + str(len(_globalAST.function_calls)) + " ; " + str(len(_globalAST.get_functions())))
